...
class OrganizationAdminRule implements AccessRuleInterface
{
    ...
    public function validateAccess(?UserInterface $user, Request $request): void
    {
        ...
        $organizationMemberId = $request->attributes->get('organizationMember');
        
        $organization = $organizationId ? $this->organizationRepository->findOrFail($organizationId) : null;
        
        if(!$organization && $organizationMemberId){
            $organizationMember = $organizationMemberId ? $this->organizationMemberRepository->findOrFail($organizationMemberId) : null;
            $organization = $organizationMember instanceof OrganizationMember ? $organizationMember->getOrganization() : null;
        }
